...
select
    ...
    equity
    + bond
    - lag(equity + bond, 1, equity + bond) over (order by snapshot_month) as total_diff,
    100.0 * equity / (equity + bond)
    - lag(100.0 * equity / (equity + bond), 1, 100.0 * equity / (equity + bond)) over (
        order by snapshot_month
    ) as equity_percentage_diff,
    100.0 * bond / (equity + bond)
    - lag(100.0 * bond / (equity + bond), 1, 100.0 * bond / (equity + bond)) over (
        order by snapshot_month
